...
class SmimeHandler
{
    ...
    public function signMessage(string $mimeBody, Certificate $cert, PrivateKey $key, array $options = []): string
    {
        ...
        try {
            if ($detached) {
                ...
                $result = $this->buildClearSignedMessage($mimeBody, $signatureDer, $hashAlgo);
            } else {
                $signedDer = $this->signer->sign($mimeBody, $cert, $key, $signOptions);
                $signedBase64 = $this->wrapBase64($signedDer);
                $result = $this->buildOpaqueSignedMessage($signedBase64);
            }

            CaLog::log('smime_sign', 'info', "CMS message: smime_sign", [
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'smime_sign', 'exception' => $e::class]);
    ...
    public function encryptMessage(string $mimeBody, array $recipientCerts, array $options = []): string
    {
        ...
        try {
            ...
            $result = $this->buildEnvelopedMessage($envelopedBase64);

            CaLog::log('smime_encrypt', 'info', "CMS message: smime_encrypt", [
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'smime_encrypt', 'exception' => $e::class]);
    ...
    public function signAndEncrypt(string $mimeBody, Certificate $cert, PrivateKey $key, array $recipientCerts): string
    {
        ...
        try {
            ...
            CaLog::log('smime_sign_and_encrypt', 'info', "CMS message: smime_sign_and_encrypt", [
                'certificate_id' => $cert->id,
                'hash_algorithm' => $hashAlgo,
                'recipient_count' => count($recipientCerts),
            ]);
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'smime_sign_and_encrypt', 'exception' => $e::class]);
    ...
    public function verifyMessage(string $smimeMessage): bool
    {
        ...
        if (str_contains($contentType, 'multipart/signed')) {
            $result = $this->verifyClearSigned($smimeMessage, $contentType);
        } elseif (str_contains($contentType, 'application/pkcs7-mime')) {
            $result = $this->verifyOpaque($smimeMessage);
        } else {
            $result = false;
        }

        CaLog::log('smime_verify', 'info', "CMS message: smime_verify", [
            'result' => $result,
    ...
    public function decryptMessage(string $smimeMessage, Certificate $cert, PrivateKey $key): string
    {
        try {
            ...
            $result = $this->encryptor->decrypt($der, $cert, $key);

            CaLog::log('smime_decrypt', 'info', "CMS message: smime_decrypt", [
                'certificate_id' => $cert->id,
            ]);

            return $result;
        } catch (\Throwable $e) {
            CaLog::critical($e->getMessage(), ['operation' => 'smime_decrypt', 'exception' => $e::class]);
