...
class CmsSigner implements CmsSignerInterface
{
    ...
    public function signDetached(string $data, Certificate $cert, PrivateKey $key, array $options = []): string
    {
        ...
        try {
            ...
            CaLog::log('cms_sign_detached', 'info', "CMS message: sign_detached", [
                'certificate_id' => $cert->id,
                'hash_algorithm' => $hashAlgo,
                'detached' => true,
            ]);
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'cms_sign_detached', 'exception' => $e::class]);
    ...
    public function addCounterSignature(string $signedDataDer, Certificate $cert, PrivateKey $key): string
    {
        try {
            ...
            $result = $this->injectCounterSignature($signedDataDer, $counterSigAttr);

            CaLog::log('cms_counter_sign', 'info', "CMS message: add_counter_signature", [
                'certificate_id' => $cert->id,
                'hash_algorithm' => $hashAlgo,
            ]);
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'cms_counter_sign', 'exception' => $e::class]);
    ...
    public function verify(string $signedDataDer, ?string $content = null): bool
    {
        ...
        CaLog::log('cms_verify', 'info', "CMS message: verify", [
            'result' => $result,
            'has_external_content' => $content !== null,
        ]);
